            ...
            for oid, centroid in tracker.objects.items():
                ...
                if len(areas)>=3:
                    ...
                    if prev_mean>0 and last_area/prev_mean>APPROACH_RATIO:
                        ...
                        text = f"A person is {approx_m:.1f} meters away, approaching from the {pos}." if approx_m else f"A person is approaching from the {pos}."
                        announcer.announce(f"approach_{oid}", text)
                # draw
                cv2.rectangle(frame_resized,(x1,y1),(x2,y2),(0,255,0),2)
                label_txt = f"person {approx_m:.1f}m" if approx_m else "person"
