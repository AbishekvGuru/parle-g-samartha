"""
real_time_assist_midas_win_fixed.py

Features:
- YOLOv8 detection via Ultralytics
- MiDaS monocular depth
- Distance estimation in meters (smoothed)
- Announces approaching objects with correct class
- Summary every 10 seconds, read aloud
- Big black text overlay
- Calibration via 'c' key
"""

import time, threading, queue, os, tempfile
...
CARE_CLASSES = {
    ...
    "bottle","cup","fork","knife","spoon","bowl","laptop","cell phone",
    "book","backpack","umbrella"
}
DEFAULT_KNOWN_DISTANCE = 1
depth_scale = 1.0
calibrated = False
    ...
    try:
        while True:
            ...
            if MIDAS_AVAILABLE and frame_idx%3==0:
                try: depth_map = run_midas(frame_rgb)
                except Exception as e: print("MiDaS error:", e); depth_map=None
            ...
            for oid, centroid in tracker.objects.items():
                ...
                if depth_map is not None:
                    midas_val = median_depth(depth_map, bbox)
                    if midas_val and midas_val>0:
                        eps=1e-6
                ...
                # get label from filtered detection
                label_name="object"
                for det in filtered:
                    x1_d,y1_d,x2_d,y2_d,det_label,_=det
                    if abs(x1-x1_d)<5 and abs(y1-y1_d)<5 and abs(x2-x2_d)<5 and abs(y2-y2_d)<5:
                ...
                if len(areas)>=3:
                    ...
                    if prev_mean>0 and last_area/prev_mean>APPROACH_RATIO:
                        ...
                        text_msg = f"{label_name.capitalize()} is {approx_m:.1f} meters away, approaching from {pos}." if approx_m else f"{label_name.capitalize()} approaching from {pos}."
                        announcer.announce(f"approach_{oid}", text_msg)

                # draw
                label_txt = f"{label_name} {approx_m:.1f}m" if approx_m else label_name
                cv2.rectangle(frame_resized,(x1,y1),(x2,y2),(0,255,0),2)
                cv2.putText(frame_resized,label_txt,(x1,max(y1-8,0)),cv2.FONT_HERSHEY_SIMPLEX,1.0,(0,0,0),3)

                summary_list.append(label_name)

            # summary every 10s
            now = time.time()
            if now - last_summary_time > SUMMARY_INTERVAL and summary_list:
                summary_text = "In the frame: " + ", ".join(set(summary_list))
                announcer.announce("summary", summary_text)
            ...
            if key==ord('q'): break
            if key==ord('c'):
                selected=None; max_area=0
                for det in filtered:
                    x1_d,y1_d,x2_d,y2_d,label,_=det
                    if label!="person": continue
                    area=(x2_d-x1_d)*(y2_d-y1_d)
                    if area>max_area: max_area=area; selected=det
                if selected and depth_map is not None:
                    bx1,by1,bx2,by2,label,_=selected
                    obs=median_depth(depth_map,(bx1,by1,bx2,by2))
                    if obs and obs>0:
                        new_scale=calibrate_known(obs,DEFAULT_KNOWN_DISTANCE)
                        if new_scale:
                        ...
                        else: set_status("Auto calibration failed")
                    else: set_status("Auto calibration failed (no depth)")
                else: set_status("Auto calibration failed (no person)")
            ...
            if calibrated:
                cv2.putText(frame_resized,f"Calibrated: scale={depth_scale:.3f} ({last_calibration_info})",(8,FRAME_H-8),cv2.FONT_HERSHEY_SIMPLEX,0.7,(0,0,0),2)
            else:
                cv2.putText(frame_resized,f"Not calibrated. Press 'c' to auto-calibrate (~{DEFAULT_KNOWN_DISTANCE} m).",(8,FRAME_H-8),cv2.FONT_HERSHEY_SIMPLEX,0.6,(0,0,0),2)
